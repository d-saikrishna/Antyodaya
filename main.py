...
def update_figure(mtric_chosen,state,index,deprivation_threshold):
    ...
    if index==None:
        ...
        df1['District_name'] = df1['District_name'].str.strip()
        df1['MAP_KEY'] = df1['MAP_KEY'].str.strip()
        df2 = pd.read_csv('Education.csv')
        df2['State_name'] = df2['State_name'].str.strip()
        df2['District_name'] = df2['District_name'].str.strip()
        df2['MAP_KEY'] = df2['MAP_KEY'].str.strip()
        df3 = pd.read_csv('Infrastructure.csv')
        df3['State_name'] = df3['State_name'].str.strip()
        df3['District_name'] = df3['District_name'].str.strip()
        df3['MAP_KEY'] = df3['MAP_KEY'].str.strip()
        df4 = pd.read_csv('Banking.csv')
        df4['State_name'] = df4['State_name'].str.strip()
        df4['District_name'] = df4['District_name'].str.strip()
        df4['MAP_KEY'] = df4['MAP_KEY'].str.strip()
        demog = ['State_name', 'District_name', 'villages_surveyed','MAP_KEY','Year', 'State_code', 'tot_pop', 'pop_male', 'pop_female', 'tot_hh']
        percentages = df1.merge(df2.drop(demog,axis=1),on='District_code').merge(df3.drop(demog,axis=1),on='District_code').merge(df4.drop(demog,axis=1),on='District_code')
        percentages = percentages.merge(mega[['District_code','Banking','Education','Infrastructure','Health','MEGA']],
                                        on=['District_code'])
    ...
    #districts_to_map = kMeans_df.copy()
    ...
    if state==None:
        ...
        state_dep = pd.DataFrame(dep_df[dep_df['State_name']==state].drop(demog_cols,axis=1).mean(),columns=['Deprivation'])
